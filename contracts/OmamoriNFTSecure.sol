...
/**
 * @title OmamoriNFTSecure
 * @notice SECURE ERC-721 contract for Omamori tablets with fair randomness
 * @dev Implements pure randomness without HYPE amount influence and proper rarity distribution
 * @author Hyper Faith Team
 */
contract OmamoriNFTSecure is ERC721, Ownable, IERC2981 {
    ...
    /**
     * @notice Mint an Omamori NFT by burning native HYPE with SECURE randomness
     * @dev Uses pure randomness without HYPE amount influence
     */
    function mint() external payable {
        ...
        // 🔒 SECURE: Generate seed WITHOUT msg.value influence
        uint64 seed = uint64(uint256(keccak256(abi.encodePacked(
            block.timestamp,
            block.prevrandao,
            msg.sender,
            tokenId
            // ✅ msg.value NOT included - pure randomness
        ))));
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        // Generate full tokenURI with SVG
    ...
    function _generateBasicTokenURI(uint256 tokenId) internal view returns (string memory) {
        ...
        string memory json = string(abi.encodePacked(
            ...
            '","description":"A mystical protection charm with fair randomness",',
            '"attributes":[',
                ...
                '{"trait_type":"Seed","value":"', _toString(seed), '"},',
    ...
    /**
     * 🔒 SECURE: Select material with proper rarity distribution (NO HYPE influence)
     * @param seed Pure random seed without HYPE amount influence
     * @return materialId Material ID with proper rarity distribution
     */
    function _selectMaterialSecure(uint64 seed) internal pure returns (uint16) {
        uint256 roll = seed % 10000; // 0.01% precision
        
        // 50% Common (Materials 0-11) - 12 materials
        if (roll < 5000) {
            return uint16(seed % 12);
        }
        
        // 25% Uncommon (Materials 12-17) - 6 materials  
        if (roll < 7500) {
            return uint16(12 + (seed % 6));
        }
        
        // 15% Rare (Materials 18-21) - 4 materials
        if (roll < 9000) {
            return uint16(18 + (seed % 4));
        }
        
        // 7.5% Ultra Rare (Materials 22-23) - 2 materials
        if (roll < 9750) {
            return uint16(22 + (seed % 2));
        }
        
        // 2.5% Mythic (Material 23) - 1 material
        return 23;
    }
    
    /**
     * @notice Get rarity name for material ID
     */
